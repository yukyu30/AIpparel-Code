...
class VisPattern(core.ParametrizedPattern):
    ...
    def serialize(self, path, to_subfolder=True, tag='', spec_only=False):
    ...
    def _save_as_image_3D(self, png_filename):
        ...
        for panel in self.pattern['panels']:
            ...
            verts_to_plot = copy.copy(verts_2d)
    ...
    def _draw_a_panel(self, drawing, panel_name, offset=[0, 0]):
        ...
        vertices = np.asarray(panel['vertices'], dtype=int).copy()
        ...
        for edge in panel['edges']:
            ...
            if ('curvature' in edge):
                control_scale = self._flip_y(copy.copy(edge['curvature']))
    ...
    def _draw_a_panel_3d(self, drawing, panel_name):
        """
        Adds a requested panel to the svg drawing with given offset and scaling
        Assumes (!!) 
            that edges are correctly oriented to form a closed loop
    ...
    def _draw_a_panel_3d(self, drawing, panel_name, offset=[0, 0]):
        """
        Adds a requested panel to the svg drawing with given offset and scaling
        Assumes (!!) 
            that edges are correctly oriented to form a closed loop
    ...
    def _save_as_boxmesh(self, boxmesh_filename):
        ...
        for panel in panel_order:
            if panel is not None:
                panel_offset_x, height = self._draw_a_panel_3d(
                    dwg, panel,
                    offset=[panel_offset_x + base_offset[0], base_offset[1]]
                )
