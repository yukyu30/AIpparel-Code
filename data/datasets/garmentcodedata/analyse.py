...
# My
import garmentcodedata.transforms as transforms
...
from garmentcodedata.panel_classes import PanelClasses
...
class GarmentCodeData(Dataset):  
    def __init__(self, root_dir, gt_caching=True, feature_caching= True, caching=True, info=False,
                 start_config={}, in_transforms=[]): 
        caching = gt_caching or feature_caching

        # precaution, taken from maria's code
        if 'mesh_samples' not in start_config:
            start_config['mesh_samples'] = 2000
        if 'point_noise_w' not in start_config:
            start_config['point_noise_w'] = 0.0

        # added from maria's code, segmentation mask if necessary
        self.segm_cached = {}

        # initialize keys for correct dataset initialization
        # precaution, taken from maria's code
        pattern_size_initialized = False
        if ('max_pattern_len' in start_config 
                and 'max_panel_len' in start_config
                and 'max_num_stitches' in start_config):
            pattern_size_initialized = True
        else: 
            start_config.update(max_pattern_len=None, max_panel_len=None, max_num_stitches=None)

        # another precaution, taken from maria's code
        if 'obj_filetag' not in start_config:
            # look for objects with this tag in filename when loading 3D models
            start_config['obj_filetag'] = 'sim'  
        ...
        self.body_type = start_config['body_type']
        self.data_folders = [ f'{df}/{self.body_type}' for df in start_config['data_folders']]
        self.data_folders_nicknames = dict(zip(self.data_folders, start_config['data_folders']))
        ...
        for data_folder in self.data_folders: 
            ...
            if self.config['panel_classification'] is None:
            ...
            #  NOTE: using max_datapoints_per_folder for easy data cut when debugging
            max_els = self.config['max_datapoints_per_folder']
            if len(clean_list) > max_els:
                clean_list = clean_list[:max_els]   # NOTE: Usefull for debugging!
        ...
        if self.config['panel_classification'] is not None:
            self.panel_classifier = PanelClasses(classes_file=self.config['panel_classification'])
        ...
        # TODO Save & Load panel classes from file properly 
        # TODO Clean this up
        # if not os.path.isfile(Path('/cluster/scratch/tkesdogan/_tmp_euler/sewformer-garments/panel_classes.pkl')):
        #     print("saving the panel classes in a pickle file")
        #     with open('/cluster/scratch/tkesdogan/_tmp_euler/sewformer-garments/panel_classes.pkl', 'wb') as f:
        #         pickle.dump(dict(self.dict_panel_classes), f)
        # if not os.path.isfile(Path('/cluster/scratch/tkesdogan/_tmp_euler/sewformer-garments/datapoints.pkl')):
        #     print("saving the valid datapoints in a pickle file")
        #     with open('/cluster/scratch/tkesdogan/_tmp_euler/sewformer-garments/datapoints.pkl', 'wb') as f:
        #         pickle.dump(self.datapoints_names, f)
        print("The panel classes in this dataset are :", self.panel_classes)
        

        if not pattern_size_initialized:
            num_panels, num_edges_in_panel, num_stitches = self._pattern_size()
        ...
        # Override by class count
        self.config['max_pattern_len'] = max(self.config['max_pattern_len'], len(self.panel_classifier))
        print("The max pattern length is :", self.config['max_pattern_len'])
        print("The max panel length is :", self.config['max_panel_len'])
        print("The max number of stitches is :", self.config['max_num_stitches'])

        self.gt_cached = {}
        self.feature_cached = {} 
        self.feature_caching = caching
        self.gt_caching = caching
        if caching: 
            print('BaseDataset::Info::Storing datapoints gt and features info in memory')
        ...
        # not needed for now, from maria's code
        # if self.config['panel_classification'] is not None:
        ...
        # semsible code, from maria's code, makes sense to keep if we want to make pattern_size 
        # a dynamic part of the config
            
        ## issue: max_pattern depends on largest pattern within the folder 
        ## - in previous version max_panel was not required as such  
        ...
        # from maria's code
        # from garment3dpatternfulldataset
        self.config.update(
            ...
            stitch_tag_size=self[0]['ground_truth']['stitch_tags'].shape[-1] if 'stitch_tags' in self[0]['ground_truth'] else 0,
            explicit_stitch_tags=False
        )

        if info: # TODO What is this and why?
            with open('/home/maria/garments/timur/information.pkl', 'wb') as handle:
                pickle.dump(self.information, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def non_empty_dirs(self, dirs, root_path):
        ...
        for dir in dirs:
            ...
            spec_list = [f for f in files if 'specification.json' in f]
            pc_list = [file for file in files if '.xyz' in file]
            # changed from obj to ply file
            obj_list = [file for file in files if self.config['obj_filetag'] in file and '.ply' in file]

            if len(spec_list) and (len(pc_list) or len(obj_list)):
    ...
    def __getitem__(self, idx):
        ...
        folder, body_type, name = tuple(datapoint_name.split('/'))
        sample = {'features': features, 'ground_truth': ground_truth, 'name': name, 'data_folder': f'{folder}/{body_type}'}
    ...
    # added from maria - latest version from the garment3dpatternfulldataset
    def _get_sample_info(self, datapoint_name):
        ...
        #     # Segmentation
        #     # FIXME This is not a great workaround, but segmentation is not used in final setup
        #     # So it's okay to have it None for now
        #     segm = None if verts is None else self._point_classes_from_mesh(
    ...
    def sample_mesh_points(num_points, verts, faces):
        ...
        points, _  = igl.random_points_on_mesh(num_points, verts, faces)
...
if __name__ == "__main__": 
    ...
    dataset = GarmentCodeData(root_dir, start_config={'random_pairs_mode': False, 'body_type' : 'default_body',
                            ...
                            'mesh_samples' : 2000,     
                            'max_datapoints_per_folder' : 10000,                   
                            'stitched_edge_pairs_num': 100, 'non_stitched_edge_pairs_num': 100,
                            'shuffle_pairs': False, 'shuffle_pairs_order': False, 
                            'data_folders': [
                            'garments_5000_0', 
                            'garments_5000_17', 
                            'garments_5000_25', 'garments_5000_33', 
                            'garments_5000_1', 'garments_5000_18', 'garments_5000_26', 'garments_5000_34', 
                            'garments_5000_10', 'garments_5000_19', 'garments_5000_27', 'garments_5000_35', 
                            'garments_5000_11', 'garments_5000_2', 'garments_5000_28', 'garments_5000_4', 
                            'garments_5000_12', 'garments_5000_20', 'garments_5000_29', 'garments_5000_5', 
                            'garments_5000_13', 'garments_5000_21', 'garments_5000_3', 'garments_5000_6', 
                            'garments_5000_14', 'garments_5000_22', 'garments_5000_30', 'garments_5000_7', 
                            'garments_5000_15', 'garments_5000_23', 'garments_5000_31', 'garments_5000_8', 
                            'garments_5000_16', 'garments_5000_24', 'garments_5000_32', 'garments_5000_9'
                            ]})
    
    scale, shift = np.zeros(7), np.zeros(7)
    min_for_dim, max_for_dim = np.zeros(7), np.zeros(7)
    dim = [[],[],[],[],[],[],[]]
    for dp in tqdm(dataset): 
        gt = dp['ground_truth']
        outlines = gt['outlines']
        num_edges = dp['ground_truth']['num_edges']
        edges = np.where(num_edges > 0)[0]
        list_edges = dp['ground_truth']['outlines'][edges][np.where(dp['ground_truth']['outlines'][edges].sum(axis=2))]
        for i in range(7): 
            dim[i].append(list_edges[:,i])
    
    all_dims = []
    for i in range(7): 
        all_dims.append(torch.concatenate(dim[i]))
        min_for_dim[i] = all_dims[i].min()
        max_for_dim[i] = all_dims[i].max()
    
    for i in range(7):
        all_dims[i] = all_dims[i].sort()[0]
    
    
    scale = max_for_dim - min_for_dim
    shift = min_for_dim
    print(scale)
    print(shift)
    import code; code.interact(local=locals())
    for i in range(7): 
        torch.save(all_dims[i], f"/home/timur/dim_{i}.pt")
    print("done")