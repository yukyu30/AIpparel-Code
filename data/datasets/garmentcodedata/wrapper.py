...
def collate_fn(batches):
    ...
    if isinstance(batches[0]["ground_truth"], dict):
        ...
        cum_sum = 2 * 108  #  56   # FIXME should come from parameters  # NOTE: 2x max_num_stitches
        for i, batch in enumerate(batches):
            for key, val in batch.items():
                if key in ["image", "name", "data_folder", "img_fn", "features"]:
...
class DatasetWrapper(object):
    ...
    def new_loaders(self, batch_size=None, shuffle_train=True, device=None, distributed=True):
        ...
        self.loaders.full = DataLoader(self.dataset, self.batch_size, collate_fn=collate_fn)
        ...
        if self.validation is not None and self.test is not None:
            ...
            try:
                ...
                # NOTE: Not needed for a new dataset
                ...
                # batch_sampler = BalancedBatchSampler(train_indices_per_type, batch_size=c)

                # NOTE: Distributed training
                if distributed:
                    training_sampler = DistributedSampler(self.training, rank=device, shuffle=True)
                else:
                ...
                self.loaders.train = DataLoader(
                    ...
                    num_workers=self.dataset.config['n_workers_dataload'] if 'n_workers_dataload' in self.dataset.config else 0,
                    pin_memory=True   # NOTE -- speeds up CPU-GPU data transfers..
                    )
            ...
            # TODO Also distributed? 
            self.loaders.validation = DataLoader(
                self.validation, 
                self.batch_size, 
                collate_fn=collate_fn,
                pin_memory=True   # NOTE -- speeds up CPU-GPU data transfers..
            ...
            self.loaders.valid_single_per_data = DataLoader(
                Subset(self.dataset, single_sample_ids), 
                collate_fn=collate_fn,
                batch_size=self.batch_size, 
                shuffle=False
            ) 
    ...
    def _loaders_dict(self, subsets_dict, batch_size, shuffle=False):
        ...
        for name, subset in subsets_dict.items():
            loaders_dict[name] = DataLoader(subset, batch_size, shuffle=shuffle, collate_fn=collate_fn)
        return loaders_dict
    ...
    def save_to_wandb(self, experiment):
        ...
        # FIXME: Doesn't work on Euler, only locally. Evaluate splits & stats locally to get the info faster!
        # experiment.save_file(experiment.local_wandb_path() / 'data_split.json')
    ...
    def predict(self, model, save_to, sections=['test'], single_batch=False, orig_folder_names=False):
        ...
        model.cuda()   # FIXME CPU? .to(device)
        model.eval()
        ...
        for section in sections:
            ...
            with torch.no_grad():
                ...
                if loader:
                    for batch in loader:
                        features_device = batch['features'].cuda()  # FIXME CPU .to(device)
