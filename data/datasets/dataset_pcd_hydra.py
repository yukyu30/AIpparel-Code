...
@dataclass 
class MeshConfig():
    mesh_samples: int = 4096 
    point_noise_w: float = 0.

@dataclass
class SplitConfig():
    type: Literal["percent", ] = 0.8

@dataclass 
class GarmentPCDDatasetConfig():
    root_dir: str
    _target_: str = "data.dataset_pcd_hydra.GarmentPCDDataset"
    gt_caching: bool = False
    feature_caching: bool = False
    panel_info: PanelConfig = field(default_factory=PanelConfig)
    standardize: StandardizeConfig = field(default_factory=StandardizeConfig)
    mesh_info: MeshConfig = field(default_factory=MeshConfig)
    include_posed_objects: bool = False
    use_front_img: bool = False
    

class GarmentPCDDataset(Dataset):
    def __init__(self, 
                 root_dir: str, 
                 gt_caching: bool=False, 
                 feature_caching: bool=False,
                 include_posed_objects: bool=False,
                 use_front_img: bool=False,
                 panel_info: PanelConfig=None,
                 standardize: StandardizeConfig=None,
                 mesh_info: MeshConfig=None):
        ...
        self.include_posed_objects=include_posed_objects
        self.use_front_img=use_front_img
        ...
        try:
            ...
            for folder in folders:
                ...
                if os.path.exists(gt_folder):
                    ...
                    if os.path.exists(object_folder) and os.path.exists(pose_folder) and self.include_posed_objects:
        ...
        log.info("Total valid datanames is {}".format(len(self)))
        ...
        if self.feature_caching:
            log.info('Storing datapoints feature info in memory')
    ...
    def save_to_wandb(self, experiment: ExperimentWrappper):
        """Save data cofiguration to current expetiment run"""
        # panel classes
        if self.panel_classifier is not None:
            shutil.copy(
                self.panel_classifier.filename, 
                os.path.join(experiment.local_output_dir, 'panel_classes.json'))
    
    def set_training(self, is_train=True):
        self.is_train = is_train
    
    def update_config(self, in_config: PanelConfig):
    ...
    def __getitem__(self, idx):
        ...
        if ground_truth["stitch_adj"] is not None:
            masked_stitches, stitch_edge_mask, reindex_stitches = self.match_edges(ground_truth["free_edges_mask"], \
                                                                                ...
                                                                                max_num_stitch_edges=self.panel_info.max_stitch_edges)
    ...
    def _sample_points(self, data_paths):
        ...
        points_and_normals = self.sample_mesh_points(self.mesh_info.mesh_samples, all_vertices, all_faces, face_normals)

        # add gaussian noise
        if self.mesh_info.point_noise_w:
            points_and_normals[:, :3] += np.random.normal(loc=0.0, scale=self.mesh_info.point_noise_w, size=points_and_normals.shape)
    ...
    def _get_pattern_ground_truth(self, gt_folder, folder_elements, spec_dict):
        ...
        patterns = self._read_pattern(
            ...
            pad_panels_to_len=self.panel_info.max_panel_len,
            pad_panel_num=self.panel_info.max_pattern_len,
            pad_stitches_num=self.panel_info.max_num_stitches,
    ...
    def _pred_to_pattern(self, prediction, dataname, return_stitches=False):
        ...
        for gt_field in fields(StandardizeConfig):
            if gt_field.name == 'stitch_tags':  
                continue
            
            pred_numpy = prediction[gt_field.name].detach().cpu().numpy()
            if gt_field.name == 'outlines' and len(pred_numpy.shape) == 2: 
                pred_numpy = pred_numpy.reshape(self.panel_info.max_pattern_len, self.panel_info.max_panel_len, 4)
            scale = getattr(self.gt_stats, gt_field.name).scale
            shift = getattr(self.gt_stats, gt_field.name).shift
            prediction[gt_field.name] = pred_numpy * scale + shift
