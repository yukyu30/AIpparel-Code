...
class GarmentCodeDatasetResize(Dataset):  
    def __init__(
        self, 
        root_dir: str, 
        size_file: str,
    ...
    def __getitem__(self, idx):
        ...
        if data_name in self.gt_cached:
            gt_pattern, pattern_dict, resized_pattern, resized_pattern_dict, size  = self.gt_cached[data_name]
        else:
            ...
            # maybe adapt paths
            resizing_spec_fil = os.path.join(self.root_path, datapoint_name.replace("default_body", "random_body"), f'{data_name}_specification_shifted.json')
            if not os.path.exists(resizing_spec_fil):
                raise Exception(f"Editing spec file {resizing_spec_fil} does not exist")
            else:
                resized_pattern = NNSewingPattern(resizing_spec_fil, panel_classifier=self.panel_classifier, template_name=self.template_name(datapoint_name))
                resized_pattern.name = data_name
                resized_pattern_dict = self.garment_tokenizer.encode(resized_pattern)
            
            size = self.size_dict[data_name]
            size = {"0":"small", "1": "medium", "2": "large", "3": "extra large"}[size]
            
            self.gt_cached[data_name] = (gt_pattern, pattern_dict, resized_pattern, resized_pattern_dict, size)
        #### DONT TOUCH END
        ...
        for i in range(1):
            question_template = f"I have a sewing pattern: {DEFAULT_PLACEHOLDER_TOKEN}. Can you resize the pattern to size {size}",
        ...
        out_pattern = [resized_pattern, gt_pattern]
        before_pattern_dict = pattern_dict
        after_pattern_dict = resized_pattern_dict
