...
def collate_fn_regression(
    ...
    for (
        ...
        pattern_endpoints_list.append(torch.cat(pattern_dict['endpoints']))
        pattern_transf_list.append(torch.cat(pattern_dict['transformations']))
    ...
    question_pattern_transfs = torch.zeros(len(question_pattern_transf_list), question_pattern_transf_max_len, pattern_transf_list[0].shape[-1], dtype=torch.float32)
    ...
    for conversation, target, pattern_id, pattern_params, qusetion_pattern_params in zip(conversation_list, targets, pattern_ids, pattern_param_list, question_pattern_param_list):
        ...
        if cur_len < tokenizer.model_max_length:
            if not (cur_len == total_len and n_patterns == len(pattern_id) == len(pattern_params)):
                print(conversation_list)
                print("cur_len: ", cur_len, "total_len: ", total_len)
                print(n_patterns, len(pattern_id), len(pattern_params))
                print(target)
                print(input_ids)
                print(image_path_list)
                print(tokenizer.decode(torch.where(input_ids[0] == IGNORE_INDEX, tokenizer.unk_token_id, input_ids[0])))
