...
from data.garment_tokenizers.special_tokens import PanelEdgeTypeV2, PanelEdgeTypeV3
from data.garment_tokenizers.utils import control_to_abs_coord
...
class NNSewingPattern(VisPattern):
    ...
    def reorder_edges(self, panel_name, shift_center=False):
        panel = self.pattern['panels'][panel_name]
        old2new_map = {}
        new_verts = []
        first_edge_id = panel['edges'][0]['endpoints'][0]
        next_edge_id = panel['edges'][0]['endpoints'][1]
        old2new_map[first_edge_id] = 0
        new_verts.append(panel['vertices'][first_edge_id])
        while next_edge_id!= first_edge_id:
            old2new_map[next_edge_id] = len(new_verts)
            new_verts.append(panel['vertices'][next_edge_id])
            for edge in panel['edges']:
                if edge['endpoints'][0] == next_edge_id:
                    next_edge_id = edge['endpoints'][1]
                    break
                
        for stiches in self.pattern['stitches']:
            for edge in stiches:
                if edge['panel'] == panel_name:
                    edge['edge'] = old2new_map[edge['edge']]
        for edge in panel['edges']:
            edge['endpoints'] = [old2new_map[edge['endpoints'][0]], old2new_map[edge['endpoints'][1]]]
        panel['vertices'] = new_verts
        if shift_center:
            trans = panel['vertices'][0]
            panel['vertices'] = np.array(panel['vertices']) - np.array(trans)
            panel['vertices'] = panel['vertices'].tolist()
            shift = np.append(trans, 0)  # to 3D
            ...
            original = panel['translation'] 
            panel['translation'] = [
                original[0] + comenpensating_shift[0], 
                original[1] + comenpensating_shift[1], 
                original[2] + comenpensating_shift[2], 
            ]

    @staticmethod
    def combine_patterns(patterns):
        combined_pattern = NNSewingPattern(panel_classifier=None, template_name="_".join([pat.template_name for pat in patterns]))
        pattern_dict = {
            'panels':{},
            'stitches':[],
            'panel_order': []
        ...
        panel_order = []
        for pat in patterns:
            porder = pat.panel_order(pad_to_len=None)
            for order in porder:
                if order is not None:
                    panel_order.append((pat.template_name, order))  
        for template_name, panel_name in panel_order:
            pat = [pat for pat in patterns if pat.template_name == template_name][0]
            pat._normalize_edge_loop(panel_name)
            pat.reorder_edges(panel_name, shift_center=True)
            panel = pat.pattern['panels'][panel_name]
            pname = pat.panel_classifier.class_name(pat.panel_classifier.class_idx(template_name, panel_name)) if pat.panel_classifier is not None else panel_name
            pattern_dict['panels'][pname] = panel
            pattern_dict['panel_order'].append(pname)
        for pat in patterns:
            stitches = pat.pattern['stitches']
            for stitch in stitches:
                edge1 = stitch[0]
                edge1_panel_name, edge1_edge_id = edge1['panel'], edge1['edge']
                edge1_panel_name = pat.panel_classifier.class_name(pat.panel_classifier.class_idx(pat.template_name, edge1_panel_name)) if pat.panel_classifier is not None else panel_name
                edge2 = stitch[1]
                edge2_panel_name, edge2_edge_id = edge2['panel'], edge2['edge']
                edge2_panel_name = pat.panel_classifier.class_name(pat.panel_classifier.class_idx(pat.template_name, edge2_panel_name)) if pat.panel_classifier is not None else panel_name
                pattern_dict['stitches'].append([
                    {'panel': edge1_panel_name, 'edge': edge1_edge_id},
                    {'panel': edge2_panel_name, 'edge': edge2_edge_id}
                ])
        combined_pattern.pattern = pattern_dict
        combined_pattern.spec['pattern'] = pattern_dict
    ...
    def panel_vertices(self, panel_name):
        ...
        for i, (edge, curvature) in enumerate(zip(edge_sequence, curvatures)):
            if not np.all(curvature == 0):
                abs_ctrl_pts.append(control_to_abs_coord(edge[0], edge[1], curvature)[None])
        all_vertices = np.concatenate([vertices] + abs_ctrl_pts, 0)
        delta = edge_sequence[0][0].copy()
        return all_vertices - delta
    
    def panel_as_list(self, panel_name, version=2, rot_in_euler=False, endpoint_first=False):
        """
            Represent panel as sequence of edges with each edge as vector of fixed length plus the info on panel placement.
            * Edges are returned in additive manner: 
                each edge as a vector that needs to be added to previous edges to get a 2D coordinate of end vertex
        ...
        all_edges = []
        delta = edge_sequence[0][0].copy()
        edge_type_version = PanelEdgeTypeV2 if version == 2 else PanelEdgeTypeV3
        for i, (edge, curvature) in enumerate(zip(edge_sequence, curvatures)):
            if np.all(curvature == 0):
                edge_type = edge_type_version.LINE if i != num_edges - 1 else edge_type_version.CLOSURE_LINE
                params = edge[1] - delta
            else:
                edge_type = edge_type_version.CURVE if i != num_edges - 1 else edge_type_version.CLOSURE_CURVE
                params = np.concatenate([control_to_abs_coord(edge[0], edge[1], curvature), edge[1]]).reshape(2, 2) \
                    if not endpoint_first else \
                        np.concatenate([edge[1], control_to_abs_coord(edge[0], edge[1], curvature)]).reshape(2, 2)
                params = params - delta
                params = params.flatten()
            if edge_type.is_closure():
                params = params.flatten()[:2] if not endpoint_first else params.flatten()[-2:]
        ...
        panel_rotation = scipy_rot.from_euler('xyz', panel['rotation'], degrees=True)  # pattern rotation follows the Maya convention: intrinsic xyz Euler Angles
        rotation_representation = np.array(panel_rotation.as_quat()) if not rot_in_euler else np.array(panel['rotation'])
