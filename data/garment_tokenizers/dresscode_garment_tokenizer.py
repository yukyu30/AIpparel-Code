...
class DressCodeGarmentTokenizer(GarmentTokenizer): 
    def __init__(self, 
        ...
        self.zero = discretize(np.zeros(2), self.bin_size, self.gt_stats.vertices.shift, self.gt_stats.vertices.scale).tolist()
    ...
    def get_all_token_names(self):
        ...
        return token_names + self.panel_names
    ...
    def encode(self, pattern: Union[GCD_NNSewingPattern, SF_SewingPattern], return_type="pt"):
        ...
        bin_idx = [self.bin_name2bin_idx[p] for p in bin_tokens]
        if self.encode_stitches_as_tags:
            tag_tokens = self.get_stitch_tag_names()
            
        pattern_edges, panel_names, panel_rotations, panel_translations, stitches = self._pattern_as_list_sf(pattern, as_quat=False, endpoint_first=True)
        stitches = self.assign_tags_to_stitches(stitches, pattern) if self.encode_stitches_as_tags else {}
        out_description = [self.special_token_indices.get_token_indices(SpecialTokensV2.PATTERN_START)]
        for panel_edges, panel_name, panel_tran, panel_rot in zip(pattern_edges, panel_names, panel_translations, panel_rotations):
            panel_name = pattern.panel_classifier.class_name(pattern.panel_classifier.class_idx(pattern.template_name, panel_name))
            out_description += [
            ...
            out_description += [self.panel_edge_type_indices.get_token_indices(PanelEdgeTypeV3.MOVE)] + [bin_idx[p] for p in trans_params.flatten().tolist()] + [bin_idx[p] for p in rot_params.flatten().tolist()]
            for edge_id, panel_edge in enumerate(panel_edges):
                ...
                out_description += [token_indices] + [bin_idx[p] for p in edge_params.flatten()[:param_num].tolist()]
                if self.encode_stitches_as_tags:
                    # last entry is null
    ...
    def decode(self, output_dict: Dict[str, Any], panel_classifier: PanelClasses): 
        ...
        pattern_dict, error_type = self.decode_pattern(pattern_tokens)
        pattern.pattern_from_pattern_dict(pattern_dict)
        return output_ids, pattern, error_type
        
    def decode_pattern(self, token_sequence: np.ndarray): 
        ...
        for i in range(n_panels):
            ...
            for j in range(len(commands)):
                ...
                num_should_predict = edge_type.get_num_params() + int(self.encode_stitches_as_tags) if edge_type != PanelEdgeTypeV3.MOVE else 6
                ...
                if edge_type == PanelEdgeTypeV3.CLOSURE_LINE:
                    ...
                    endpoint, abs_ctrl_pt = edge_params[0], edge_params[1]
