...
from data.garment_tokenizers.utils import arc_rad_flags_to_three_point, control_to_abs_coord, control_to_relative_coord, arc_from_three_points, panel_universal_transtation, is_colinear
from scipy.spatial.transform import Rotation
from data.datasets.utils import IMAGE_TOKEN_INDEX
from data.datasets.panel_configs import *
from data.garment_tokenizers.special_tokens import SpecialTokens, PanelEdgeType, PanelEdgeTypeIndices, SpecialTokensIndices, DecodeErrorTypes

class GCDGarmentTokenizer: 
    def __init__(self, 
    ...
    def get_all_token_names(self):
        return PanelEdgeType.list() + SpecialTokens.list() + self.get_stitch_tag_names()
    ...
    def encode(self, pattern: NNSewingPattern, return_type="pt"):
        ...
        stitches = self.assign_tags_to_stitches(stitches)
    ...
    def decode_pattern(self, token_sequence: np.ndarray, param_dict: Dict[int, np.ndarray], tokenizer: PreTrainedTokenizer): 
        assert self.special_token_indices is not None \
        ...
        pattern: Dict[str, List] = {'panels': {},'stitches': [], 'panel_order':[]}
        edge_stitches: Dict[int, List] = {}
        for i in range(n_panels):
            panel_dict: Dict[str, List] = {
                'rotation': [0, 0, 0],
                'translation': [0, 0, 0],
                'vertices': [[0, 0]],
                'edges': []
    ...
    def evaluate_patterns(self, pred_patterns: List[NNSewingPattern], gt_patterns: List[NNSewingPattern]):
        ...
        for k, (pred_pattern, gt_pattern) in enumerate(zip(pred_patterns, gt_patterns)):
            ...
            for i in range(len(pred_panel_names)):
                try:
                    edges, rot, transl, _ = pred_pattern.panel_as_numeric(pred_panel_names[i], pad_to_len=None)
                except EmptyPanelError:
                    continue
                if pred_panel_names[i] in gt_panel_names:
                    gt_edges, gt_rot, gt_transl, _ = gt_pattern.panel_as_numeric(pred_panel_names[i], pad_to_len=None)
                else:
                    continue
            ...
            for i in range(len(pred_panel_names)):
                edges, rot, transl, _ = pred_pattern.panel_as_numeric(pred_panel_names[i], pad_to_len=None)
                gt_edges, gt_rot, gt_transl, _ = gt_pattern.panel_as_numeric(gt_panel_names[i], pad_to_len=None)
