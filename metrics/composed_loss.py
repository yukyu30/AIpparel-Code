...
class ComposedPatternLoss():
    ...
    def __init__(self, data_config, in_config={}):
        ...
        self.config = {  # defults
            ...
            'quality_components': ['2d_chamfer', 'shape'],  # 'loop',  
            'loop_loss_weight': 1.,
    ...
    def _main_losses(self, preds, ground_truth, gt_num_edges):
        ...
        if 'kl' in self.l_components and 'posterior' in preds:
            kl_loss = preds['posterior'].kl(dims=(1, 2)).mean()
            full_loss += self.config['kl_weight'] * kl_loss
    ...
    def _main_quality_metrics(self, preds, ground_truth, gt_num_edges, names):
        ...
        correct_mask = None
        if 'discrete' in self.q_components:
            num_panels_acc, num_edges_acc, correct_mask, num_edges_correct_acc, out_patterns, valid_panel_num, valid_edge_num = self.pattern_nums_quality(
        ...
        if "2d_chamfer" in self.q_components:
            ...
            chamfer_l2 = self.chamfer_quality(point_samples.to(out_patterns.device), gt_point_samples, valid_panel_num, ground_truth['num_panels'].to(torch.int32))
