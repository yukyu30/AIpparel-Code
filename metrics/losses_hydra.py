...
import entmax # https://github.com/deep-spin/entmax
# My modules
from metrics.eval_detr_metrics import eval_pad_vector
from data.datasets.dataset_pcd_hydra import StatsConfig

# ----- Utils -----
def eval_pad_vector(data_stats: Optional[StatsConfig]=None) -> Optional[torch.Tensor]:
...
# ------- custom losses --------
class SparsemaxLoss(entmax.SparsemaxLoss):
    def __init__(self, weight: float=1.0):
        super().__init__()
        self.weight=weight
...
class PanelLoopLoss(BaseLoss):
    """Evaluate loss for the panel edge sequence representation property: 
        ensuring edges within panel loop & return to origin"""
    def __init__(self, standardize_config: Optional[StatsConfig]=None, weight: float=1.0):
        super().__init__(weight)
        self.pad_vector = eval_pad_vector(standardize_config)

            
    def __call__(self, predicted_panels: torch.Tensor, gt_panel_num_edges: Optional[torch.Tensor]=None):
...
class PatternStitchLoss(BaseLoss):
    ...
    """
    def __init__(self, triplet_margin=0.1, use_hardnet=True, weight: float=1.0):
        super().__init__(weight)
