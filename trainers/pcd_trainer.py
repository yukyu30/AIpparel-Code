...
class TrainerPCD(Trainer):
    ...
    def _add_optimizer(self, model_without_ddp):
        param_dicts = [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "pcd_encoder" not in n and p.requires_grad]},
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "pcd_encoder" in n and p.requires_grad],
                "lr": float(self.setup["lr_pcd_encoder"]),
            },
        ]
    ...
    def _fit_loop_without_matcher(self, model, criterion, train_loader, valid_loader, start_epoch):
        ...
        for epoch in range(start_epoch, wb.config.trainer["epochs"]):
            ...
            for i, batch in enumerate(train_loader):
                ...
                if self.scheduler is not None and False:
                    self.scheduler.step()
