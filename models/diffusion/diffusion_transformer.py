...
from models.utils.transformer_blocks import (
    init_linear,
    MLP,
...
class DitBlock(nn.Module):
    def __init__(
        ...
        self.ln_1 = AdaLayerNorm(width)

        if context_dim is not None:
            self.ln_2 = AdaLayerNorm(width)
...
def build(args):
    model = UNetDiffusionTransformer(
        n_ctx=args["n_ctx"],
        width=args["width"],
        layers=args["layers"],
        heads=args["heads"],
