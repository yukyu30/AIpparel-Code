...
from ..decoders.transformer import build_transformer, TransformerDecoder, TransformerDecoderLayer, Transformer
...
class GarmentPCD(nn.Module):
    def __init__(self, pcd_encoder: AlignedShapeLatentPerceiver, panel_transformer: Transformer, num_panel_queries: int, num_edges: int, posterior_dim: int, **edge_kwargs):
        super().__init__()
...
class SetCriterionWithOutMatcher(nn.Module):
    ...
    def __init__(self, data_config, in_config={}):
        ...
        self.config = {}
        self.config['loss'] = {
            'loss_components': ['shape', 'loop', 'rotation', 'translation', 'kl'],  # , 'stitch', 'free_class'],
...
def build(args) -> Tuple[GarmentPCD, SetCriterionWithOutMatcher]:
    ...
    shape_encoder = build_shape_encoder(args)
    posterior_dim = args["NN"].get("src_dim", None)
    model = GarmentPCD(shape_encoder, panel_transformer, num_classes, 14, posterior_dim if posterior_dim is not None else panel_transformer.d_model , edge_kwargs=args["NN"])
    if args["NN"].get("pretrained_first_stage_model_path", None):
        ckpt = torch.load(args["NN"]["pretrained_first_stage_model_path"], map_location=devices)["model_state_dict"]
        ckpt = {k.replace("module.", ""): v for k, v in ckpt.items()}
        model.load_state_dict(ckpt)
        print("Load pretrained model from ", args["NN"]["pretrained_first_stage_model_path"])
...
if __name__ == "__main__":
    import pyyaml
    import ipdb; ipdb.set_trace()
    arg_path = pyyaml.load(open("/home/w4756677/garment/sewformer/Sewformer/configs/train_pcd2garment/train.yaml", "r"), Loader=pyyaml.FullLoader)
