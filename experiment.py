...
os.environ['WANDB_DIR'] = "/miele/george/garment/sewformer"
os.environ['WANDB_CACHE_DIR'] = "/miele/george/garment/sewformer/cache"
import torch
...
class ExperimentWrappper(object):
    ...
    def save_checkpoint(self, state, aliases=[], wait_for_upload=False):
        ...
        past_filename = self.checkpoint_filename(self.checkpoint_counter - 1)
        ...
        artifact.add_file(str(self.local_artifact_path() / filename), skip_cache=True, policy="immutable")
        wb.run.log_artifact(artifact, aliases=['latest'] + aliases)
        self._wait_for_upload(self.artifactname('checkpoint', version=self.checkpoint_counter))
        api = wb.Api({'project': self.project})
        for v in api.artifacts("checkpoint", self.artifactname('checkpoint', with_version=False)):
            # Clean up versions that don't have an alias such as 'latest'.
            # NOTE: You can put whatever deletion logic you want here.
            if len(v.aliases) == 0:
                print("Experiment::Cleaning up artifact {} without alias".format(v.name))
                v.delete()
